# SP2L Indicator for MetaTrader 5 (English)

A faithful MQL5 port of the **SP2L strategy** (Spike + Pullback + Gap) by
...
2. Go to `MQL5\Indicators\` and copy `SP2L_Indicator.mq5` there.
3. Open **MetaEditor** (F4 from MT5), open `MQL5\Indicators\SP2L_Indicator.mq5`
   and press **Compile** (F7). It should show *0 errors*.
4. Back in MT5, open **Navigator → Indicators** (refresh if needed) and drag
...
- **Advanced** (default) - exact port of `SP2L_Advanced_Bot.py`:
  ...
  - **pending setup** that waits for the first valid entry candle
    (low/high break), exactly like the bot,
